...
SELECT d.element_id,
  ...
  FROM gvt_fatura_minima      f,
       ...
       descriptions           de
 WHERE d.bill_ref_no = 161213913 --v_ign_index_bill_ref
...
select A.BILL_REF_NO,
 ...
 where a.bill_ref_no = 161213913
   ...
   and a.element_id in (10203,10327)
   
   -- Type 1  Element 10203   Component 29649
   -- Type 1  Element 10327   Component 29897
   
select * from bill_invoice_detail where bill_ref_no = 161213913 and type_code = 2 and element_id in (10203)
...
select A.BILL_REF_NO,
  ...
  from bill_invoice_detail a,
       descriptions b
 where 1 =1 --a.bill_ref_no = 161213913
   and A.DESCRIPTION_CODE = B.DESCRIPTION_CODE
   and B.LANGUAGE_CODE = 2
   and upper(B.DESCRIPTION_TEXT) like ('%GVT NA MEDIDA%')
