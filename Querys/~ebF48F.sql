 ...
 CURSOR C_BUSCA_DUPLICIDADE(bill_no number) is -- C2
        select pk.COMPONENT_ID, BI.BILL_REF_NO, BI.ACCOUNT_NO, VL.DISPLAY_VALUE, PK.SUBSCR_NO, PK.FROM_DATE, count(1) total
         ...
         where PK.BILL_REF_NO = BI.BILL_REF_NO
           ...
           and PK.COMPONENT_ID in (SELECT to_number(COMPONENT_ID) FROM g0023421sql.GVT_VAL_PLANO where NOME_PLANO = 'TOUCHE')
 ...
 CURSOR C_BUSCA_PROD_PLANO(bill_no number, subs_no number) is -- C3
          select cm.*,EQ.EXTERNAL_ID,
            ...
            where cm.bill_ref_no = bill_no
              ...
              and cm.component_id not in (SELECT to_number(COMPONENT_ID)FROM g0023421sql.GVT_VAL_PLANO where NOME_PLANO = 'TOUCHE' and COMPONENT_ID is not null)
              and cm.component_id in (SELECT to_number(COMPONENT_ID)FROM g0023421sql.GVT_VAL_PLANO where NOME_PLANO = 'LAVOISIER' and COMPONENT_ID is not null);
