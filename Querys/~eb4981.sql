      SELECT to_char(NULL),      to_char(NULL),    package_id,         to_number(NULL),                        type_code,
             ...
             to_char(NULL),      to_number(NULL),  d.discount_level,
                          to_number(NULL),       to_number(NULL),  to_number(NULL), to_char(NULL), to_number(0), to_number(0), to_number(0)
      ...
      SELECT to_char(NULL),      to_char(NULL),        package_id,         component_id,                           type_code,
             ...
             DECODE(b.type_code, 5, cck1.AVAIL_PERIODS, 6, cck2.AVAIL_PERIODS, null), to_char(NULL), to_number(0), to_number(0), to_number(0)
      ...
      SELECT to_char(NULL),      to_char(NULL),      package_id,           component_id,                           type_code,
             ...
             p.description_code, b.tax_type_code,    b.bill_invoice_row,   decode( 2, 1, tracking_date, 2, NULL ), to_number(NULL),
             to_char(NULL),      to_number(NULL),    d.discount_level,
                      to_number(NULL),       to_number(NULL),    to_number(NULL), r.region, to_number(0), to_number(0), to_number(0)
      ...
      WHERE  bed.bill_invoice_row   (+) = b.bill_invoice_row
        ...
        AND  d.discount_id          (+) = b.discount_id
                ...
                AND  ct2.contract_type      (+) = cc2.contract_type)
      UNION ALL
      SELECT c.external_id,      m.external_id,          package_id,           b.component_id,                           type_code,
             subtype_code,       b.tracking_id,          b.tracking_id_serv,   prep_sequence,                          prorate_code,
             billing_level,      billing_category,       b.amount,             b.units,                                tax,
             tax_rate,           tax_pkg_inst_id,        discount,             b.discount_id,                          d.description_code,
             trans_date,         b.provider_id,          b.element_id,         b.product_line_id,                      b.description_code,
             ...
             p.description_code, b.tax_type_code,        b.bill_invoice_row,   decode( 2, 1, tracking_date, 2, NULL ), to_number(NULL),
             to_char(NULL),      s.parent_account_no,   d.discount_level,
                      to_number(NULL),       to_number(NULL),    to_number(NULL),     r.region, nvl(cck.commitment_amount,0), nvl(ct.duration,0), nvl(ct.contract_category,0)
      FROM   BILL_INVOICE_DETAIL b,  CUSTOMER_ID_ACCT_MAP c, SERVICE s, CUSTOMER_ID_EQUIP_MAP m, PRODUCT_LINES p,
             DISCOUNT_DEFINITIONS d, GVT_BILL_PRODUCT_RATE_KEY r, PRODUCT pr, CUSTOMER_CONTRACT cc, CUSTOMER_CONTRACT_KEY cck, CONTRACT_TYPES ct
      WHERE  b.bill_ref_no           = :p_bill_ref_no
        AND  b.bill_ref_resets          = :p_bill_ref_resets
        AND  b.billing_level         = 1
        AND  b.type_code             = 2
        ...
        AND  d.discount_id       (+) = b.discount_id
                ...
                AND     b.tracking_id             = r.tracking_id (+)
                AND  b.tracking_id_serv         = r.tracking_id_serv (+)
        AND  b.tracking_id           = pr.tracking_id
        AND  b.tracking_id_serv      = pr.tracking_id_serv
        AND  pr.contract_tracking_id      = cc.tracking_id (+) 
        AND  pr.contract_tracking_id_serv = cc.tracking_id_serv (+) 
        AND  pr.contract_tracking_id      = cck.tracking_id (+) 
        AND  pr.contract_tracking_id_serv = cck.tracking_id_serv (+) 
