...
CURSOR FATURA IS
  SELECT   MAP.EXTERNAL_ID,
   ...
   WHERE   d.bill_ref_resets = 0
    ...
    UTL_FILE.PUT_LINE( V_FILE, 'EXTERNAL_ID' ||' ; '|| 'BILL_PERIOD' ||' ; '||'ELEMENT_ID' ||' ; '|| 'BILL_REF_NO' ||' ; '|| 'TYPE_CODE' ||' ; '|| 'SUBTYPE_CODE' ||' ; '|| 'DESCRIPTION_TEXT' ||' ; '|| 'VALOR' ||' ; '|| 'VALOR_LIQ' ||' ; '|| 'DISCOUNT' ||' ; '|| 'FEDERAL_TAX' ||' ; '|| 'BILL_INVOICE_ROW' ||' ; '|| 'SUBSCR_NO' ||' ; '|| 'TAX' ||' ; '|| 'TAX_PKG_INST_ID' ||' ; '|| 'TAX_RATE' ||' ; '|| 'RATE_TYPE' ||' ; '|| 'APARECE_NOTA' ||' ; '|| 'COD_FISCAL' ||' ; '|| 'TIPO_UTILIZACAO' ||' ; '|| 'SUBTYPE_OUTRAS' ||' ; '|| 'COD_DESCONTO' ||' ; '|| 'PREP_DATE');
    
        FOR C1 IN FATURA LOOP
            BEGIN
                UTL_FILE.PUT_LINE( V_FILE, C1.EXTERNAL_ID ||' ; '|| C1.BILL_PERIOD ||' ; '|| C1.ELEMENT_ID ||' ; '|| C1.BILL_REF_NO ||' ; '|| C1.TYPE_CODE ||' ; '|| C1.SUBTYPE_CODE ||' ; '|| C1.DESCRIPTION_TEXT ||' ; '|| C1.VALOR ||' ; '|| C1.VALOR_LIQ ||' ; '|| C1.DISCOUNT ||' ; '|| C1.FEDERAL_TAX ||' ; '|| C1.BILL_INVOICE_ROW ||' ; '|| C1.SUBSCR_NO ||' ; '|| C1.TAX ||' ; '|| C1.TAX_PKG_INST_ID ||' ; '|| C1.TAX_RATE ||'  ; '|| C1.RATE_TYPE ||' ; '|| C1.APARECE_NOTA ||' ; '|| C1.COD_FISCAL ||' ; '|| C1.TIPO_UTILIZACAO ||' ; '|| C1.SUBTYPE_OUTRAS ||' ; '|| C1.COD_DESCONTO ||' ; '|| C1.PREP_DATE);
            END;
        END LOOP;
