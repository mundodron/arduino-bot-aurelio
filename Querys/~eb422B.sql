SELECT CLIENTE.PARENT_ID, 
 ...
 TB_ICMS.TAX_RATE/10000  ICMS,
 TB_PIS.TAX_RATE /10000  PIS
 FROM 
 ARBOR.CMF                       CLIENTE,
 ...
 (SELECT B.TAX_RATE, A.BILL_INVOICE_ROW
    ...
    WHERE A.BILL_REF_NO = 244013133
      ...
      AND B.FEDERAL_TAX > 0
      AND UPPER(C.DISPLAY_VALUE) LIKE '%ICMS%') TB_ICMS,
 (SELECT B.TAX_RATE, A.BILL_INVOICE_ROW
     FROM ARBOR.BILL_INVOICE_DETAIL A,
          ARBOR.BILL_INVOICE_TAX B,
          ARBOR.TAX_PKG_INST_ID_VALUES C
    WHERE A.BILL_REF_NO = 244013133
      ...
      AND C.LANGUAGE_CODE = 2
      AND B.FEDERAL_TAX > 0
      AND UPPER(C.DISPLAY_VALUE) LIKE '%PIS%') TB_PIS
WHERE CLIENTE.ACCOUNT_NO              =2572681
  ...
  AND DETALHE.BILL_INVOICE_ROW=TB_ICMS.BILL_INVOICE_ROW  (+)    
  AND DETALHE.BILL_INVOICE_ROW=TB_PIS.BILL_INVOICE_ROW   (+)
