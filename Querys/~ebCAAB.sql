...
SELECT   distinct(MAP.EXTERNAL_ID),
 ...
 CURSOR C_BUSCA_PROD_FALTANDO(acc_no number, sub_no number, cc_id number) is -- C4
        SELECT   A.COMPONENT_ID, TIRA_ACENTO(A.COMPONENTE) AS COMPONENTE
          FROM   G0023421SQL.GVT_VAL_PLANO A
         WHERE   A.PLANO = (SELECT PLANO FROM GVT_VAL_PLANO WHERE COMPONENT_ID = CC_ID) 
...
begin
    ...
    for c1 in C_FATURA loop
        ...
        for c4 in C_BUSCA_PROD_FALTANDO(c1.account_no, c1.SUBSCR_NO, C1.COMPONENT_ID) loop
