...
1.03    g0023421 Aurelio     06/07/2015              RDM23636  Arquivos TCOE para a operadora EMBRATEL centralizados na EOT 001.                                                        
---------------------------------------------------------------------------------------------------- */
iUltima_RDM                   varchar2(030) := 'RDM23636, 06/07/2015';
...
------------------- ARQUIVO DE LOG
------------------------------
APELIDO_OPERADORA             varchar2(030)  := 'EMBRATEL';
...
BEGIN
  ...
  v_SQL := 'Select min(trans_dt)  menor_trans_dt, '||
                    ...
                    'count(1)       qtde, '||
                    'eot_pagadora, '||
           'case '''||APELIDO_OPERADORA||''' when ''TELEFONICA'' then ''011''  when ''EMBRATEL'' then ''001'' else eot_pagadora end  eot_default '||
              ...
              'where status = ''N'' '||
               ...
               'group by eot_pagadora, ' ||
           'case '''||APELIDO_OPERADORA||''' when ''TELEFONICA'' then ''011''  when ''EMBRATEL'' then ''001'' else eot_pagadora end';
  
  LOCALIZACAO_ANTERIOR(15);
  
  -- grava_arq_log('FILE', 'SQL....: ' || v_SQL );
  ------------------------------------------------------------------------------
  -- HEADER
