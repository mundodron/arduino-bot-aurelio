...
SELECT   MAP.EXTERNAL_ID,
   ...
   WHERE   d.bill_ref_resets = b.bill_ref_resets
    ...
    dbms_output.put_line( V_FILE, 'Inicio - ' || AUX_GLOBAL_NAME || ' : ' || to_char( sysdate ,'dd/mm/yyyy - hh24:mi:ss'));
    dbms_output.put_line( V_FILE, 'EXTERNAL_ID' ||' ; '|| 'ACCOUNT_NO' ||' ; '|| 'BILL_PERIOD' ||' ; '|| 'BILL_STATE' ||' ; '|| 'ELEMENT_ID' ||' ; '|| 'BILL_REF_NO' ||' ; '|| 'EQUIP' ||' ; '|| 'TYPE_CODE' ||' ; '|| 'SUBTYPE_CODE' ||' ; '|| 'PRODUTO' ||' ; '|| 'VALOR' ||' ; '|| 'VALOR_LIQ' ||' ; '|| 'PAY_METHOD' ||' ; '|| 'FROM_DATE' ||' ; '|| 'TO_DATE' ||' ; '|| 'PREP_DATE' ||' ; '|| 'SAFRA' ||' ; '|| 'DISCOUNT NVL' ||' ; '|| 'DISCOUNT');
    
        FOR C1 IN FATURA LOOP
            BEGIN
                dbms_output.put_line( V_FILE, C1.EXTERNAL_ID ||' ; '|| C1.ACCOUNT_NO ||' ; '|| C1.BILL_PERIOD ||' ; '|| C1.BILL_STATE ||' ; '|| C1.ELEMENT_ID ||' ; '|| C1.BILL_REF_NO ||' ; '|| C1.EQUIP ||' ; '|| C1.TYPE_CODE ||' ; '|| C1.SUBTYPE_CODE ||' ; '|| C1.PRODUTO ||' ; '|| C1.VALOR ||' ; '|| C1.VALOR_LIQ ||' ; '|| C1.PAY_METHOD ||' ; '|| C1.FROM_DATE ||' ; '|| C1.TO_DATE ||' ; '|| C1.PREP_DATE ||' ; '|| C1.SAFRA ||' ; '|| C1.DISCOUNT_NVL ||' ; '|| C1.DISCOUNT);
            END;
        END LOOP;
