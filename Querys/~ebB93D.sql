      SELECT   DISTINCT
               ...
               NULL AS SERVICE_END,
               's/' AS DEGRAU,
                     replace(replace(RTRIM (REPLACE (UPPER (GPV.VELOCITY), ',', '.'), 'KMBGT '),'.0',null),'.00',null) VELOCIDADE,
       ...
       WHERE   SERVICE_ADDRESS_ASSOC.address_id = LOCAL_ADDRESS.address_id
               ...
               --AND   to_number (replace(replace(RTRIM (REPLACE (UPPER (GPV.VELOCITY), ',', '.'), 'KMBGT '),'.0',null),'.00')) > 1000
