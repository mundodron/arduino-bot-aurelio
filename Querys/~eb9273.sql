...
SELECT /*+ parallel (c 30)*/
  ...
  FROM   cdr_data a,
         cdr_billed b,
         (  SELECT   bill.account_no, bill.bill_ref_no, bill.bill_ref_resets
             ...
             WHERE   prep_Date BETWEEN TO_DATE ('01/04/2014 00:00:00','dd/mm/yyyy hh24:mi:ss')
                     ...
                     AND comp.component_id IN (30491, 30492)
                     AND bill.prep_status = 1
                     AND bill.prep_error_code IS NULL
          GROUP BY   bill.account_no, bill.bill_ref_no, bill.bill_ref_resets) c
