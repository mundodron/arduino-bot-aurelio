DECLARE
 ...
 where MAP.EXTERNAL_ID in (select EXTERNAL_ID from ativos)
   ...
   and BILL.ORIG_PPDD_DATE > trunc(sysdate - 30) and  BILL.PPDD_DATE < trunc(sysdate)
...
BEGIN
   ...
   LOOP
      ...
      THEN
         BEGIN
           update ativos set bill_ref_no_30 = x.Fatura, total_pago_30 = x.Total_pago
...
select count(1) from ativos

-- update ativos set account_no = null 

-- delete ativos where account_no is null

-- select * from ativos where external_id = '999996806005'
