...
drop table g0023421sql.gvt_faturas_sempre_local;

create table g0023421sql.gvt_faturas_sempre_local as
select account_no,bill_ref_no,bill_ref_resets 
from bill_invoice
where prep_Date between to_date('01/05/2014 00:00:00','dd/mm/yyyy hh24:mi:ss') and to_date('31/05/2014 23:59:59','dd/mm/yyyy hh24:mi:ss')
  and prep_status=1
  and bill_period = 'M28'
...
drop table g0023421sql.gvt_faturas_sempre_temp;

create table g0023421sql.gvt_faturas_sempre_temp as
select a.*
from g0023421sql.gvt_faturas_sempre_local a,cmf_package_component b
where a.account_no=b.parent_account_no
  and b.component_id in (30491); 
  
----------------- Eliminação de Duplicados 
drop table g0023421sql.gvt_faturas_sempre_loc;

create table g0023421sql.gvt_faturas_sempre_loc as 
select distinct a.* 
from g0023421sql.gvt_faturas_sempre_temp a;
...
from cdr_data a,cdr_billed b,g0023421sql.gvt_faturas_sempre_loc c
...
alter table g0023421sql.gvt_sempreloc_full add (jurisdiction_new number(10));


--Update para jurisdição não atendida para cidade atendida      
update g0023421sql.gvt_sempreloc_full a set type_id_usg_new = '288',jurisdiction_new=11 where type_id_usg=288 and jurisdiction=67;   

--Separação de uso SEMPRE LOCAL ATENDIDA
update g0023421sql.gvt_sempreloc_full a set type_id_usg_new = '288',jurisdiction_new=11 where type_id_usg=276;    
...
drop table g0023421sql.gvt_sempre_local_full;

create table g0023421sql.gvt_sempre_local_full as
select a.account_no,
       a.msg_id,
       a.msg_id2,
       a.open_item_id,
...
--select * from g0023421sql.gvt_sempre_local_full