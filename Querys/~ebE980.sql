-- ********************************************************************************************************************************
-- Script      : gerador_arquivos_bancarios.sql
-- Created     : 26/05/2004 - Billing Desenvolvimento - Antonio Rafael
-- Description : Gera os arquivos bancarios referente a INCLUSAO e EXCLUSAO dos clientes em DACC.
-- Alteration  : 12/08/2004 - Antes de criar os eventos de cada cenario, verifico se o cliente esta no ARBOR.
-- Alteration  : 25/01/2005 - Billing Desenvolvimento - Antonio Rafael
--                             Alteracao dos UPDATEs na TAB GVT_DACC_GERENCIA_FILA_EVENTOS
-- Alteration  : 28/11/2005 - Pequenos Desenvolvimento - Luciano Xavier da Veiga
--                              Retirada da alteração provisória para não geração do registro D para determinados bancos
-- Alteration  : 19/04/2006 - Pequenos Desenvolvimento - Luciano Xavier da Veiga
--                             Alterada geração de arquivo do Banco Real, para que fosse gerado o arquivo tipo de D de exclusão e
--                             alterado a forma como a pl atualiza os registros da fila de eventos, agora ela atualiza todos os 
--                             registros que estão "a processar" e não apenas o próximo da sequencia e deixa o status ativo para 
--                             exclusões de DACC que irão para Fatura ou DACC. A verificação dos bancos que podem não dar retorno
--                             tambem foi excluida, a GVT não ira esperar retorno de nenhum banco.
--LAST MODIFIED: 23/01/2006 - MAYCON WENDRECHOWSKI
--               INCLUSAO NOVOS BANCOS PARA DEB AUTOM, CADASTRO BILATERAL BANRISUL, BRB, BANSICREDI E BANCOOB.
--LAST MODIFIED: 23/05/2007 - MAYCON WENDRECHOWSKI
--               EXCLUSAO DA CONCATENACAO DO CAMPO CC PARA OS BANCOS BANSICREDI, BANCOOB E BANRISUL.
--LAST MODIFIED: 18/07/2007 - ALINE CAMILA CLEMENTE DA FONSECA
--               ALTERACAO DO 6° CAMPO DOS REGISTROS DO TIPO 'D', PARA QUE SEJAM ENVIADOS EM COM ESPACOS EM BRANCO,
--               PARA INCLUSAO E EXLUSAO DO DEBITO AUTOMATICO. MODIFICADO SOMENTE PARA O BANRISUL.
--LAST MODIFIED: 07/08/2007 - ALINE CAMILA CLEMENTE DA FONSECA
--                             ALTERADO O CODIGO DE CONVENIO DO BANCO SAFRA DE  'A17111' PARA 'A1000007111'
--LAST MODIFIED: 13/08/2007 - ALINE CAMILA CLEMENTE DA FONSECA
--               ALTERACAO DO 6° CAMPO DOS REGISTROS DO TIPO 'D', PARA QUE SEJAM ENVIADOS COM EXTERNAL_ID,
--               PARA INCLUSAO E EXLUSAO DO DEBITO AUTOMATICO. MODIFICADO SOMENTE PARA O BANRISUL.
--LAST MODIFIED: 08/12/2007 - ALINE CAMILA CLEMENTE DA FONSECA
--               INICIO DO CONTROLE DE ARQUIVOS DE DACC COM O REGISTRO TIPO 'J' E DA TABELA GVT_DACC_CONTROLE_NSA.
--LAST MODIFIED: 16/05/2008 - ALINE CAMILA CLEMENTE DA FONSECA
--               REMOVIDO CONTROLE DE ARQUIVOS DE DACC COM O REGISTRO TIPO 'J' E DA TABELA GVT_DACC_CONTROLE_NSA PARA O BANRISUL.
--LAST MODIFIED: 14/09/2011 - EVELIZE AMARAL DOS SANTOS
--               RFC 313127 - Alteração da data de cadastro de dacc de sysdate+6 para sysdate+1 (REGISTROS TIPO E).
-- ********************************************************************************************************************************
SET VERIFY OFF
...
------------------------------------------
  ...
  v_seq_numb             NUMBER(6);
  v_bank_seqs            VARCHAR2 (50) := 'ARBORGVT_BILLING.BANK_SEQS';
...
BEGIN
  ...
  SELECT 'DBAREM_' || GLOBAL_NAME || '.bank_seqs' into v_bank_seqs FROM GLOBAL_NAME;
  
  -- dbms_output.put_line('Server = ' || v_bank_seqs );
      ...
      IF (v_count_external_id >= 1) THEN
        ...
        IF (TRIM (r_linha.cod_agente_arrecadador) = '001') THEN
          IF v_cabecalho_arq001 = 'N' THEN
            ...
            EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = TO_NUMBER (:cod_agente_arrecadador) '
                         INTO  v_seq_numb
...
-------------------------------------------------
    ...
    if (v_cabecalho_arq001 = 'S') then
            ...
            EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 001 '
                         INTO  v_seq_numb;
    ...
    if (v_cabecalho_arq104 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 104 '
                         INTO  v_seq_numb;   
    ...
    if (v_cabecalho_arq070 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 070 '
                         INTO  v_seq_numb;  
    ...
    if (v_cabecalho_arq422 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 422 '
                         INTO  v_seq_numb;   
        ...
        insert into gvt_dacc_controle_nsa (clearing_house_id, nsa, sent_dt, received_dt) values ('422', v_seq_numb, sysdate, null);
        commit;
    ...
    if (v_cabecalho_arq237 = 'S') then
                    ...
                    EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 237 '
                         INTO  v_seq_numb;  
    ...
    if (v_cabecalho_arq275 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 275 '
                         INTO  v_seq_numb;   
    ...
    if (v_cabecalho_arq341 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 341 '
                         INTO  v_seq_numb;  
    ...
    if (v_cabecalho_arq399 = 'S') then
                    ...
                    EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 399 '
                         INTO  v_seq_numb;  
    ...
    if (v_cabecalho_arq409 = 'S') then
                ...
                EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 409 '
                         INTO  v_seq_numb;   
    ...
    if (v_cabecalho_arq745 = 'S') then
                    ...
                    EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 745 '
                         INTO  v_seq_numb;
  ...
  --bansicredi s/a
    if (v_cabecalho_arq748 = 'S') then
                    ...
                    EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 748 '
                         INTO  v_seq_numb;
  ...
  --bancoob s/a
      if (v_cabecalho_arq756 = 'S') THEN
                   ...
                   EXECUTE IMMEDIATE ' SELECT seq_numb FROM ' || v_bank_seqs || ' WHERE bank_id = 756 '
                         INTO  v_seq_numb; 
