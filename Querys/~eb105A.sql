...
----------------------------------------------------------------------------------------------------
3.    INC810202 - Na procedure “proc_carga_sp”, corrigir o valor da duração Real, para que a Rotina de Sobreposição (mesma procedure) considere a duração real, e não mais a tarifada.

2.    Na procedure “Proc_carga_sp”, fazer com que o campo “grc_servicos_prestados.eot” receba o nome do arquivo “REM.TCOE...” (completo).
E as procedures (proc_envia_contestados, proc_envia_rejeitados, proc_envia_parceiro) passem a utilizam este campo para envia o nome do arquivo até o “.NC”.
Abrir Problema e RDM para o Incidente: xxxxxxxx

*/
iUltima_RDM        varchar2(20) := 'RDM12140,v10.11.2014';
-------------------
...
PROCEDURE VALIDA_SOBREPOSICAO IS
  ...
  Cursor qLote
    ...
    Select to_date(to_char(data_servico,'yyyymmdd')||hora_inicio,'yyyymmddhh24:mi:ss') inicio_chamada,
           to_date(to_char(data_servico,'yyyymmdd')||hora_inicio,'yyyymmddhh24:mi:ss') + ((duracao*60)/86400)  fim_chamada,
           to_char(to_date('2000','yyyy') + ((duracao*60)/86400),'hh24:mi:ss') duracao_chamada,
           rowid,
           sequencial_chave,
           telefone_origem,
           telefone_destino,
      ...
      ELSE
        ...
        ELSE
          ...
          BEGIN
            ...
            v_SP.GRUPO_HORARIO       := rtrim(ltrim(substr(v_linha_arq, 237, 1))); 
            v_SP.COD_NAC_AREA_VISITADA     := rtrim(ltrim(substr(v_linha_arq, 238, 2))); 

            -- realDuration            -->    DURACAO_REAL varchar(6)
            -- billingPeriod         -->    GRUPO_HORARIO varchar(1)
            -- visitedEOT            -->    COD_NAC_AREA_VISITADA varchar(2)
            -- originalBeginChar    -->    CÓDIGO DE REGISTRO varchar(1)
            -- degree                -->    DEGRAU varchar(2)
            ...
            IF V_SP.STATUS = 'AF' THEN
              ...
              INSERT INTO GRC_SERVICOS_PRESTADOS
                   ...
                   ( v_sp.sequencial_chave,   v_sp.valor_faturado,     v_sp.data_servico,       v_sp.hora_inicio,            v_sp.duracao,           v_sp.telefone_origem,
                     ...
                     v_sp.servico_operadora,  v_sp.sub_csp,            v_sp.Data_Lote,          v_SP.DURACAO_REAL,           v_SP.GRUPO_HORARIO,     v_SP.COD_NAC_AREA_VISITADA
                   );
            ELSE
