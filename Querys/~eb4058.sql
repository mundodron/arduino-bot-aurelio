...
select * from cdr_billed where account_no = 4643727 and bill_ref_no = 165269902
...
select A.BILL_REF_NO,
  ...
  from bill_invoice_detail a,
       descriptions b
 where a.bill_ref_no = 165269902
   and A.DESCRIPTION_CODE = B.DESCRIPTION_CODE
   and B.LANGUAGE_CODE = 2
   
   and a.component_id in (29649,29897)
   
select * from bill_invoice_detail where bill_ref_no = 165269902

 and type_code = 2 and element_id in (10203,10327)
...
select * from cmf_balance where bill_ref_no = 165269902