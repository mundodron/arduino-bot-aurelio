...
WHERE CLIENTE.ACCOUNT_NO            =2871360
...
GROUP BY
 ...
 select * from customer_id_acct_map where external_id = '777777765142'
 
 select * from cmf_balance where bill_ref_no = 231925743
 
 select account_no, ACCOUNT_CATEGORY, CUST_FRANCHISE_TAX_CODE from CMF where account_no = 2871360
 
 select * from FRANCHISE_CODE_VALUES where FRANCHISE_CODE = 1 and language_code = 2
 
 select * from ACCOUNT_CATEGORY_VALUES where account_category = 12 and language_code = 2
 
 select * from bill_invoice fatura
  where FATURA.BILL_REF_NO              =231925743
    --AND FATURA.BILL_REF_NO              =DETALHE.BILL_REF_NO
    AND FATURA.ACCOUNT_NO               = 2871360
    AND FATURA.PREP_STATUS              =1
    AND FATURA.BACKOUT_STATUS           ='0'
    AND FATURA.FORMAT_ERROR_CODE        IS NULL
    AND FATURA.PREP_ERROR_CODE          IS NULL
