...
DECLARE
   ...
   BEGIN
      ...
      v_errmsg := NULL;
      v_value := 0;
       FOR tmp_ARQ_PROC IN (select * from cmf_balance where bill_ref_no in (131130801,132013137,131150937,131561917,132001277,131525903,130847213,130849054))
       loop                                     
          v_errcode := 0;
          v_errmsg := NULL;
          begin                    
          ...
          EXCEPTION
            WHEN OTHERS
            THEN
              DBMS_OUTPUT.put_line (  'Não encontrou fatura ' || tmp_ARQ_PROC.bill_ref_no || ' conta = ' || tmp_ARQ_PROC.account_no  ); 
          end;         
