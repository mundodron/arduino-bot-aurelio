select count(*) from arbor.RATE_USAGE where SEQNUM >= 96616965 and SEQNUM <= 96618314  

Validação após aplicação dos Inits

select * from arbor.rate_usage where type_id_usg in (620, 621, 623) and 
...
select * from rate_usage where type_id_usg in (620, 621, 623) and 
element_id in (10621,10624,10627,10630,10633,10636,10639,10642,10645,10648,10658,10661,10664,10667,10670)
