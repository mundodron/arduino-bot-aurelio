select 'SIEBEL' SISTEMA, to_char(s2.x_acct_id_num) CLIENTE, to_char(s3.x_acct_id_num)AGREGADORA, to_char(s.x_acct_id_num) COBRANÇA, 
...
from s_org_ext s, s_org_ext s2, s_org_ext s3
where s.x_acct_id_num in ('999982152279')
and s.master_ou_id = s2.row_id
and s.par_ou_id = s3.row_id;

select * from s_org_ext where x_acct_id_num = '999982152279'