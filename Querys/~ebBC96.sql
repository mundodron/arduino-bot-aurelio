...
Select MAP.EXTERNAL_ID, CMF.BILL_LNAME, CMF.BILL_PERIOD , bil.bill_ref_no, CMF.ACCOUNT_CATEGORY, bil.prep_date, PC.PROCESSAMENTO, CMF.NO_BILL, MAP.INACTIVE_DATE, CB.CLOSED_DATE
  from bill_invoice bil, cmf, GVT_PROCESSAMENTO_CICLO PC, customer_id_acct_map map, cmf_balance cb
 where bil.prep_date > '01/03/2016'
   and bil.prep_status = 1
   and BIL.ACCOUNT_NO = CMF.ACCOUNT_NO
   and bil.prep_error_code is null
   and MAP.ACCOUNT_NO = CB.ACCOUNT_NO
   and BIL.BILL_REF_NO = CB.BILL_REF_NO
