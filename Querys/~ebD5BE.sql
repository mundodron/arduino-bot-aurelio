...
SELECT
   ...
   WHERE oo.account_no in (select account_no from SINCRONISMO_SK)
     AND oo.order_number in (select SK.NUMERO_ORDEM from SINCRONISMO_SK sk )
     ...
     AND oi.member_type = 10
     --and oo.account_no = 3818290
     order by 1
     
  -- WHERE oo.account_no = 2412068
   --  AND oo.order_number = '8-21503899574-1'
   
   
   select count(1) from SINCRONISMO_SK sk group by SK.ACCOUNT_NO