...
--   wdirspool_in                 
   ...
   LOOP
      BEGIN
         ...
         DBMS_OUTPUT.put_line ('v_cd_banco = '||v_cd_banco);
         v_cd_tipopagto2            := SUBSTR (aux_arquivo, 38, 1);
         DBMS_OUTPUT.put_line ('v_cd_tipopagto2 = '||v_cd_tipopagto2);
         v_ds_banco                 := SUBSTR (aux_arquivo, 4, 20);
         DBMS_OUTPUT.put_line ('v_ds_banco = '||v_ds_banco);
         v_seq_nsa                  := SUBSTR (aux_arquivo, 34, 4);
         DBMS_OUTPUT.put_line ('v_seq_nsa = '||v_seq_nsa);
         v_dt_pagto                 :=
                SUBSTR (aux_arquivo, 45, 2) || '/' || SUBSTR (aux_arquivo, 43, 2) || '/'
                || SUBSTR (aux_arquivo, 39, 4);   -- Dt débito
         DBMS_OUTPUT.put_line ('v_dt_pagto = '||v_dt_pagto);
