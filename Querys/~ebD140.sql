...
SELECT d.element_id,
  ...
  FROM gvt_fatura_minima      f,
       ...
       descriptions           de
 WHERE d.bill_ref_no = 146880736 --v_ign_index_bill_ref
...
select B.DESCRIPTION_TEXT,
  ...
  from bill_invoice_detail A,
       descriptions B
 where bill_ref_no = 146880736
   and A.DESCRIPTION_CODE = B.DESCRIPTION_CODE
   and B.LANGUAGE_CODE = 2

select * from descriptions where description_code = 17199