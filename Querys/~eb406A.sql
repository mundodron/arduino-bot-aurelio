Declare 
    V_ACCOUNT_NO number(10) := 3244873; --?;
    V_BILL_REF_NO number(10) := 244009334; --?;
    V_PREP_STATUS number(10) := 1;-- ?;
...
FROM  
 ...
 (SELECT 
    ...
    WHERE A.BILL_REF_NO = V_BILL_REF_NO -- 244009334
      ...
      AND B.FEDERAL_TAX > 0
      AND UPPER(C.DISPLAY_VALUE) LIKE '%ICMS%') ICMS_TB,
  (SELECT   
         B.BILL_REF_NO
        ,B.FEDERAL_RATE 
        ,C.SHORT_DISPLAY 
    ...
    WHERE A.BILL_REF_NO = V_BILL_REF_NO -- 244009334
      ...
      AND C.LANGUAGE_CODE = 2
      AND B.FEDERAL_TAX > 0
      AND UPPER(C.DISPLAY_VALUE) LIKE '%PIS%') PIS_TB
WHERE 
    CLIENTE.ACCOUNT_NO              = V_ACCOUNT_NO -- 3244873
...
AND FATURA.BILL_REF_NO              = V_BILL_REF_NO -- 244009334
AND FATURA.BILL_REF_NO              =DETALHE.BILL_REF_NO
AND FATURA.ACCOUNT_NO               =CONTA.ACCOUNT_NO
AND FATURA.PREP_STATUS              = V_PREP_STATUS -- 1
