/* Formatted on 2010/04/29 18:13 (Formatter Plus v4.8.6) */
--******************************************************************************
--* NOME............: pl6010.sql                                                                                  
--* DESCRICAO.......: 
...
--   wdirspool_in                  := '1';
   ...
   LOOP
      BEGIN
		 ...
		 DBMS_OUTPUT.put_line(aux_arquivo);
	    ...
	    --testa se a data esta zerada
		IF (SUBSTR (aux_arquivo, 39, 8) = '00000000') then 
			DBMS_OUTPUT.put_line ('Entrou no if  1');		    
			DBMS_OUTPUT.put_line(SUBSTR(aux_arquivo, 39, 8));
			DBMS_OUTPUT.put_line(v_dt_pagto);
			
			-- v_dt_pagto := SUBSTR (aux_arquivo, 75, 2) || '/' || SUBSTR (aux_arquivo, 73, 2) || '/' || SUBSTR (aux_arquivo, 69, 4);
         v_dt_pagto                 :=
                SUBSTR (aux_arquivo, 75, 2) || '/' || SUBSTR (aux_arquivo, 73, 2) || '/'
                || SUBSTR (aux_arquivo, 69, 4);
