         ...
         AND EXISTS (SELECT 1
                      ...
                      WHERE service.parent_account_no = cmf.account_no
                       -- AND service.service_inactive_dt IS NULL)
                        ...
                        AND (service.service_inactive_dt is null or (service.service_inactive_dt + 7) > nvl(cmf.prev_cutoff_date,cmf.date_active) and nvl(cmf.prev_cutoff_date,cmf.date_active) > (sysdate - 90))                              
                     )
         AND EXISTS (SELECT 1
...
select count(*) from BIPP15_TESTE a
where not exists (select 1 from BIPP15_TESTE_prod b where b.account_no = a.account_no) --210852

-- truncate table BIPP15_TESTE_prod

-- truncate table BIPP15_TESTE


grant all on BIPP15_TESTE_prod to public