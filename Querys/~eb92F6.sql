  ...
  select * from cmf where account_no in (select account_no from chamados)
  and CMF.ACCOUNT_NO not in (  
  select account_no from adj where bill_ref_no in (select fatura from chamados)
  )
  ...
  ALTER TABLE G0023421SQL.CHAMADOS
ADD (external_id VARCHAR2(12 BYTE));
  
  select * from bmf where account_no = 3526157
  
  
  SELECT * FROM GVT_DEPARA_AJUSTE_MASSIVO WHERE UPPER(DESCRIPTION_PGTO) LIKE 'BOLETO%'
