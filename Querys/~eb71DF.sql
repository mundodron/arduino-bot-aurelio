...
SELECT   distinct(MAP.EXTERNAL_ID),
 ...
 CURSOR C_BUSCA_PROD_PLANO(bill_no number, subs_no number) is -- C3
          select cm.*,EQ.EXTERNAL_ID,
            ...
            where cm.bill_ref_no = bill_no
              ...
              and cm.component_id not in (SELECT to_number(COMPONENT_ID)FROM g0023421sql.GVT_VAL_PLANO where NOME_PLANO = 'LAVOISIER')
 ...
 CURSOR C_BUSCA_PROD_FALTANDO(acc_no number, bill_no number, cc_id number, subs_cc number) is -- C4
        SELECT   A.COMPONENT_ID, TIRA_ACENTO(A.COMPONENTE) AS COMPONENTE
         ...
         WHERE   A.PLANO = (SELECT PLANO FROM g0023421sql.GVT_VAL_PLANO WHERE NOME_PLANO = 'LAVOISIER' AND COMPONENT_ID = cc_id) 
           AND   A.TIPOELEMENTO = 'RC - Conta'
           AND   A.COMPONENT_ID not in (30487,25011,30989,30490)
                 AND NOT EXISTS
                       (SELECT   1
        ...
        SELECT A.COMPONENT_ID, TIRA_ACENTO(A.COMPONENTE) AS COMPONENTE
         ...
         WHERE A.PLANO = (SELECT PLANO FROM g0023421sql.GVT_VAL_PLANO WHERE NOME_PLANO = 'LAVOISIER' AND COMPONENT_ID = CC_ID) 
           ...
           AND A.COMPONENT_ID not in (30491,30492,30488,30679,30490)
                 AND NOT EXISTS
                       (SELECT   1
                          FROM   bill_invoice_detail CPC,
                                 bill_invoice BB
