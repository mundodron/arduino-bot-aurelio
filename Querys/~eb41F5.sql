...
--          b.bill_ref_resets,
          ...
          b.statement_date,
          b.payment_due_date,
          substr(CONVERT( replace(replace(DECODE(TRIM(c.bill_lname) , NULL,' ', SUBSTR(TRIM(c.bill_lname), 1,54 )) ,CHR(13),''),CHR(10),''), 'US7ASCII', 'WE8ISO8859P1'),1,54),
          c.account_category,
          d.external_id,
          0 valor_ajustes,
          0 total_adj, 
   ...
   AND    b.prep_status = 1
   AND    TO_CHAR(b.statement_date, 'YYYYMM') <= TRIM(to_char(sysdate,'YYYYMM'))
   AND    nome_programa = 'PLCON_0001'
   AND    flg_utilizado = 'N'
   AND    b.prep_date+0 <= TO_DATE(TO_CHAR(LAST_DAY(TO_DATE(RTRIM(to_char(sysdate,'YYYYMM')),'YYYYMM')),'YYYYMMDD')||'235959','YYYYMMDDHH24MISS')
   ...
   OR     TO_CHAR(a.closed_date, 'YYYYMM') > TRIM(to_char(sysdate,'YYYYMM')))
