...
SELECT   MAP.EXTERNAL_ID,
   ...
   WHERE   d.bill_ref_resets = b.bill_ref_resets
      ...
      ---------------------------
           ...
           select (select tira_acento(description_text) from descriptions dt where dt.description_code = d.description_code and dt.language_code = 2) "PRODUTO",
                   d.*
             from bill_invoice_detail d
            where D.SUBSCR_NO = 8977545
              and d.bill_ref_no = 188385851
              and COMPONENT_ID in (SELECT to_number(COMPONENT_ID) FROM GVT_VAL_PLANO)
           ...
           select * from COMPONENT_DEFINITION_VALUES where language_code = 2
