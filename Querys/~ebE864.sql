...
select * from gvt_febraban_accounts where external_id = 999979767269
...
WHERE service.parent_ACCOUNT_NO in (select account_no from customer_id_acct_map where external_id in ('999979767269'))
...
select Q.ROW_ID, A.EXTERNAL_ID as CMF_EXT_ID, C.EXTERNAL_ID as EMF_EXT_ID, A.ACCOUNT_NO, 
 ...
 where A.ACCOUNT_NO = S.PARENT_ACCOUNT_NO
   ...
   and A.EXTERNAL_ID = '999979767269'
   and X.X_ACCT_ID_NUM = A.EXTERNAL_ID
   and X.x_acct_id_num = '999979767269'
   and P.EXTERNAL_ID = A.EXTERNAL_ID
   and C.EXTERNAL_ID = q.ASSET_NUM
   
select * from customer_id_acct_map where external_id = '999979767269'

select * from gvt_febraban_accounts where external_id = '999979767269'

select bill_ref_no, File_name from gvt_febraban_bill_invoice where account_no = 7933910 and bill_ref_no in (170162710,167037724)
   
select * from GVT_FEBRABAN_PONTA_B_ARBOR where EMF_EXT_ID in ('999979767269')

select * from gvt_febraban_ponta_b_arbor where emf_ext_id = 'SPO-301EBL070-032'

select * from gvt_febraban_ponta_b_arbor where emf_ext_id = 'BHE-301J2CSKE-032'
