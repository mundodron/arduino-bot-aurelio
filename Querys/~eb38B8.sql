...
-- Data:     Abril/2009
-- Gera informações para disponibilizar na Web a conta detalhada para clientes Retail.
----------------------------------------------------------------------------------------------
-- Versão: Autor:               Data:       Doc:        Motivo:
   ------  -------------------- ----------- ----------- --------------------------------------
-- 1.1     Valter (g0010388)    18/Feb/2010 RFC 156351  Omitir a linhas em branco supérfluas.
-- 1.2     g0010388 Valter      31/MAR/2010 RFC 164552  Gerar arquivo de LOG.
-- 1.3     P9906496 Daniel		  28/JUN/2011             Inclusão de serviços de TV
-- 1.4     g0010388 Valter      31/AUG/2011 RFC 329629  Correção de Erro character to number conversion error
-- 1.5     g0007405 Evelize     03/OUT/2012 RFC 390828  Correção de cálculo dos descontos.
----------------------------------------------------------------------------------------------
  ...
  IF p_tipo = 'FILE'  OR  p_tipo is null THEN
	  if not UTL_FILE.IS_OPEN(yFileERRO) then
	    ...
	    utl_file.put_line(yFileERRO,
	        'plCONTA_DETALHADA_P_C - RETAIL - RELATÓRIO DE LOG - Gerado em:'||to_char(Time,'dd/mm/yyyy hh24:mi:ss')||CHR(10)||chr(10)||
      ...
      FOR y IN chamadas LOOP
       ...
       --IF (y.type_id_usg <> v_type_id_usg_aux) OR ( y.external_id <> TRIM(v_ext_id_aux) AND TRIM(v_ext_id_aux) <> 0 ) THEN
         IF (y.type_id_usg <> v_type_id_usg_aux) OR ( y.external_id <> TRIM(v_ext_id_aux) AND TRIM(v_ext_id_aux) <> '0' ) THEN -- RFC 329629
          /* Quando foi sumarizado todos os valores de uma mesma instância e tipo de uso
         ...
         --dbms_output.put_line(t_type_id_orig || '#' || v_swap || '#' || y.point_origin || '#' ||y.point_target );
          ...
          iLocal := substr(iLocal,1,instr(iLocal,'~~')-1)||'~~ 16';
                ...
                (v_arq_ponto_conta,
									...
									|| CHR(09)
									|| 'Duração:DuracCham:time:0,0:time'
									|| CHR(09)
									...
									|| CHR(09)
									|| 'Tipo Ligação:TipoLigCham:string:100,0:text'
									|| CHR(09)
									|| 'Tipo:TipoCham:string:80,0:text'
									|| CHR(09)
									|| 'Horário:HorarioCham:string:40,0:text'
									|| CHR(09)
         ...
         IF v_flag_mensalidade = 'S' THEN
            ...
            UTL_FILE.put_line
               (v_arq_ponto_conta,
                ...
                || CHR(09)
                || 'Descrição:DescServMens:string:80,0:text'
                || CHR(09)
                || 'Data de início:DataIniSerMens:date:0,0:date'
                || CHR(09)
...
------------------------------------------------------------------------------------------------
/***
**** procedimento para gravar o detalhamento de serviços eventuais
***/
   ...
   BEGIN
      ...
      FOR y IN mensalidades LOOP
       IF ( y.provider_id = v_provider_id AND ( y.type_code = 3) and (y.open_item_id <> 2 and y.open_item_id <> 3 )) THEN
         IF v_flag_nrc = 'S' THEN
            ...
            UTL_FILE.put_line (v_arq_ponto_conta,chr(09));
            UTL_FILE.put_line (v_arq_ponto_conta, 'Serviços Eventuais'|| CHR (09) ||'ServEvent'|| CHR (09) ||'nao');
            UTL_FILE.put_line (v_arq_ponto_conta,chr(09));
            ...
            UTL_FILE.put_line
               (v_arq_ponto_conta,
                ...
                || CHR(09)
                || 'Descrição:DescServEve:string:80,0:text'
                || CHR(09)
      ...
      -- Incluido para Gerar dados de Usos de TV na parte de Serviços Eventuais
...
***/
   ...
   BEGIN
      ...
      UTL_FILE.put_line (v_arq_ponto_conta, 'Encargos'|| CHR (09) ||'Encargos'|| CHR (09) ||'nao');
            ...
            UTL_FILE.put_line
               (v_arq_ponto_conta,
                   'Descrição:DescrEncargo:string:80,0:text'
                || CHR(09)
      ...
      for d in descontos loop
      	IF ( d.discount_id is not null) THEN
        	IF v_flag_desconto = 'S' THEN
      		...
      		UTL_FILE.put_line
          		     (v_arq_ponto_conta,
           		      'Descrição:DescrDesconto:string:80,0:text'
          		      || CHR(09)
...
BEGIN
  ...
  BEGIN
	...
	IF v_existe_fatura = 1 THEN
		...
		exception
		  when no_data_found then
			GRAVA_ARQ_LOG('FILE', 'Fatura não possui Nota Fiscal.');
    ...
    IF v_existe_fatura = 1 THEN
      ...
      if not eOPERADORA then
        GRAVA_ARQ_LOG('FILE', 'Não existem Usos.');
  ...
  GRAVA_ARQ_LOG('', '.'||lpad(yqtde_sem_fatura ,8,' ')||' contas não tinham fatura.');
  GRAVA_ARQ_LOG('', '.'||lpad(yqtde_sem_nota   ,8,' ')||' contas não tinham nota fiscal.');
  GRAVA_ARQ_LOG('', '.'||lpad(yqtde_sem_usos   ,8,' ')||' contas não tinham CDRs.');
  end if;
