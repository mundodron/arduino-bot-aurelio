...
------  -------------------- ----------- ----------- --------- --------------------------------------
 ...
 1.9    Aurelio Avanzi       09/07/15    REQ1037491  RDM23921  Chamadas contestadas cujo motivo for igual a 218 são inseridas na grc_erros_bmp
----------------------------------------------------------------------------------------------------
*/

iUltima_RDM        varchar2(20) := 'RDM23921,v09.07.2015';
...
FUNCTION NAOFATURACONJUNTO (p_Telefone varchar2 ) RETURN varchar2 IS
 V_VALOR varchar2(15) := null;
BEGIN
     SELECT telefone into V_VALOR
       FROM GRCOWN.GRC_NAOFATURACONJUNTO
      WHERE telefone = p_Telefone
	    AND NVL(operadora, ' ') = v_sp.csp
    ...
    EXCEPTION
     WHEN NO_DATA_FOUND THEN
         RETURN (null); 
     WHEN OTHERS THEN
         grava_arq_log('', 'Erro ao executar NAOFATURACONJUNTO >>> '||replace(sqlerrm, chr(10), '; ') );
    RAISE; 
END NAOFATURACONJUNTO;
    ...
    BEGIN
      ...
      ELSE
        ...
        --v_sp.lote := substr(file_name_tcoe, instr(file_name_tcoe, 'TCOE'), 24); -- RFC 346359
          ...
          BEGIN
           IF  V_SP.STATUS = 'AF' and NAOFATURACONJUNTO(v_sp.telefone_origem) is null THEN
            ...
            ELSE
              ...
              INSERT INTO GRC_ERROS_BMP
                    ...
                    (v_sp.sequencial_chave,   v_sp.valor_faturado,     v_sp.data_servico,       v_sp.hora_inicio,            duracao_erro,      v_sp.telefone_origem,
                     ...
                     v_sp.natureza,           v_sp.pais_destino,       v_sp.cod_seq_TFI,        v_sp.cod_contestacao,        v_sp.lote,              v_sp.motivo,-- verificar código com reginaldo
