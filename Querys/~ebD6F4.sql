      ...
      WHERE  b.bill_ref_no              = 246231919 --:p_Orig_bill_refno
        AND  b.bill_ref_resets          = 0 --:p_Orig_bill_ref_resets
      ...
      WHERE  b.bill_ref_no              = 246231919 -- :p_Orig_bill_refno
        ...
        AND  m.is_current               = 1
        AND  l.country_code             = ccv.country_code     (+)
        AND  ccv.language_code (+)      = 0 --:p_language_code
        AND  s.emf_config_id            = cfg.emf_config_id    (+)
        AND  cfg.language_code (+)      = 0 -- :p_language_code
        AND  m.external_id_type           = 6    
        AND  sb.equip_class_code        = c.equip_class_code   (+)
        AND  c.language_code   (+)      = 0 -- :p_language_code
        AND  s.rate_class                = r.rate_class
        AND  s.emf_config_id            = rec.emf_config_id    (+)
        AND  rec.language_code (+)      =  0 --:p_language_code
      ORDER  BY si, sir;
