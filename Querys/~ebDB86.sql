     SELECT   DISTINCT
               ...
               NULL AS SERVICE_END,
               's/' AS DEGRAU,
               nvl(case when trim(translate (replace(replace(RTRIM (REPLACE (UPPER (GPV.VELOCITY), ',', '.'), 'KMBGT '),'.0',null),'.00',null), '0123456789', ' ')) is null then replace(replace(RTRIM (REPLACE (UPPER (GPV.VELOCITY), ',', '.'), 'KMBGT '),'.0',null),'.00',null) else '0' end,0) VELOCIDADE,
               LTRIM (nvl(GPV.VELOCITY,'na'), '1234567890.') AS UNID_VELOCIDADE,
